...
class TecnovigilanciaService {
  ...
  /**
   * Helper: Convert empty strings to null for enum fields
   */
  _cleanData(data) {
    const cleaned = { ...data };
    // Convert empty strings to null for optional fields
    const optionalFields = [
      'fabricante', 'modelo', 'numeroSerie', 'lote', 'registroSanitario',
      'tipoEvento', 'gravedadEvento', 'desenlace', 'accionesTomadas',
      'observaciones', 'numeroReporteINVIMA', 'fechaReporteINVIMA', 'estado'
    ];
  ...
  async create(data, userId) {
    ...
    const reporte = await prisma.reporteTecnovigilancia.create({
      data: {
        ...cleanedData,
        reportadoPor: userId,
        estado: cleanedData.estado || 'BORRADOR',
      },
      include: {
        paciente: {
          select: {
  ...
  async getConsolidadoTrimestral(trimestre, anio) {
    if (!trimestre || !anio) {
      throw new ValidationError('Trimestre y año son requeridos');
    }
