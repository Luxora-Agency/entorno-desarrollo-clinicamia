...
class OrdenMedicaPdfService {
  ...
  async generarOrdenPdf(ordenId) {
    ...
    doc.text(`Fecha: ${format(new Date(orden.fechaOrden), 'dd/MM/yyyy HH:mm', { locale: es })}`, { align: 'right' });
    doc.moveDown();
    ...
    if (paciente) {
      ...
      if (paciente.fechaNacimiento) {
        doc.text(`Fecha Nacimiento: ${format(new Date(paciente.fechaNacimiento), 'dd/MM/yyyy')}`);
      }
    ...
    const tipoOrden = orden.examenProcedimiento?.tipo || 'No especificado';
    const badgeColor = this._getTipoColor(tipoOrden);
    ...
    doc.font('Helvetica').text(orden.examenProcedimiento?.nombre || orden.observaciones || 'No especificado');
    ...
    if (orden.resultados || orden.estado === 'Completada') {
      ...
      if (orden.fechaEjecucion) {
        doc.moveDown(0.5);
        doc.text(`Fecha de ejecución: ${format(new Date(orden.fechaEjecucion), 'dd/MM/yyyy HH:mm', { locale: es })}`);
      }
  ...
  _addFooter(doc) {
    ...
    doc.text(
      `Documento generado el ${format(new Date(), 'dd/MM/yyyy HH:mm', { locale: es })} | Este documento es válido únicamente con firma del médico tratante`,
      50,
      pageHeight - 50,
