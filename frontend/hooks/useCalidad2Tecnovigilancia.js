...
export function useCalidad2Tecnovigilancia() {
  ...
  /**
   * Load tecnovigilancia reports
   */
  const loadReportes = useCallback(async (params = {}) => {
    try {
      setLoading(true);
      const response = await apiGet('/calidad2/medicamentos/tecnovigilancia', params);
      ...
      toast({
        title: 'Error',
        description: 'No se pudieron cargar los reportes de tecnovigilancia.',
        variant: 'destructive',
      });
      return null;
    } finally {
  ...
  const getReporte = useCallback(async (id) => {
    try {
      const response = await apiGet(`/calidad2/medicamentos/tecnovigilancia/${id}`);
  ...
  const getEstadisticas = useCallback(async (filters = {}) => {
    try {
      const response = await apiGet('/calidad2/medicamentos/tecnovigilancia/estadisticas', filters);
   ...
   */
  const getConsolidado = useCallback(async (trimestre, anio) => {
    try {
      setLoading(true);
      const response = await apiGet(`/calidad2/medicamentos/tecnovigilancia/consolidado/${trimestre}/${anio}`);
      setConsolidado(response.data);
      return response.data;
    } catch (error) {
      console.error('Error getting consolidado:', error);
      toast({
        title: 'Error',
        description: 'No se pudo cargar el consolidado trimestral.',
        variant: 'destructive',
      });
      return null;
    } finally {
  ...
  const getPendientesINVIMA = useCallback(async () => {
    try {
      const response = await apiGet('/calidad2/medicamentos/tecnovigilancia/pendientes-invima');
      return response.data;
    } catch (error) {
  ...
  const createReporte = useCallback(async (data) => {
    try {
      const response = await apiPost('/calidad2/medicamentos/tecnovigilancia', data);
      ...
      toast({
        ...
        description: 'El reporte de tecnovigilancia se ha creado correctamente.',
  ...
  const updateReporte = useCallback(async (id, data) => {
    try {
      const response = await apiPut(`/calidad2/medicamentos/tecnovigilancia/${id}`, data);
  ...
  const deleteReporte = useCallback(async (id) => {
    try {
      await apiDelete(`/calidad2/medicamentos/tecnovigilancia/${id}`);
  ...
  const reportarINVIMA = useCallback(async (id, numeroReporte) => {
    try {
      const response = await apiPost(`/calidad2/medicamentos/tecnovigilancia/${id}/reportar-invima`, {
  ...
  const uploadDocumento = useCallback(async (reporteId, file, metadata = {}) => {
    try {
      ...
      const response = await apiPost(`/calidad2/medicamentos/tecnovigilancia/${reporteId}/documentos`, {
  ...
  const deleteDocumento = useCallback(async (documentoId) => {
    try {
      await apiDelete(`/calidad2/medicamentos/tecnovigilancia/documentos/${documentoId}`);
