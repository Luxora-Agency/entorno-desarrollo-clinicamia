...
export default function InventarioMedicamentosTab({ user }) {
  const {
    inventario,
    loading,
    loadInventario,
    deleteItem,
  } = useCalidad2InventarioMedicamentos('MEDICAMENTO');
  ...
  const filteredItems = inventario.filter((item) => {
    const matchesSearch = !searchTerm ||
      ...
      item.principioActivo?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="space-y-4">
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                placeholder="Buscar por código, nombre, principio activo o laboratorio..."
                value={searchTerm}
      ...
      <Dialog open={showForm} onOpenChange={() => handleCloseForm(false)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? 'Editar Medicamento' : 'Nuevo Medicamento'}
            </DialogTitle>
          </DialogHeader>
          <InventarioForm
            item={editingItem}
