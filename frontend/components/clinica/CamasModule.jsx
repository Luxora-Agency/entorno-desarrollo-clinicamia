...
import { Bed, Plus, Edit, Trash2, Filter } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-500">Cargando camas...</p>
      </div>
    );
  }
      ...
      <Card>
        ...
        <CardContent>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-green-600">
                  {camas.filter(c => c.estado === 'Disponible').length}
                </div>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-red-600">
                  {camas.filter(c => c.estado === 'Ocupada').length}
                </div>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-yellow-600">
                  {camas.filter(c => c.estado === 'Mantenimiento').length}
                </div>
          ...
          {camasFiltradas.length === 0 ? (
            <p className="text-center text-gray-500 py-8">No hay camas registradas</p>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody>
                  {camasFiltradas.map((cama) => (
                    <tr key={cama.id} className="border-b hover:bg-gray-50">
                      <td className="py-3 px-4 font-medium">{cama.numero}</td>
                      <td className="py-3 px-4">Hab. {cama.habitacion?.numero || '-'}</td>
                      <td className="py-3 px-4">{cama.habitacion?.unidad?.nombre || '-'}</td>
                      <td className="py-3 px-4">
                        <Badge variant={getEstadoBadgeVariant(cama.estado)}>
                          {cama.estado}
                        </Badge>
                      </td>
