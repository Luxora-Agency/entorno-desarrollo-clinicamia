...
export default function BuscadorPacientes({ onSelectPaciente }) {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 text-center">
        ...
        <p className="text-gray-600">
          Ingresa el nombre, cédula o correo del paciente
        </p>
      </div>
          ...
          {results.length > 0 ? (
            <>
              ...
              {results.map((paciente) => (
                <Card
                  key={paciente.id}
                  className="hover:shadow-md transition-shadow cursor-pointer border-2 hover:border-emerald-300"
                  onClick={() => onSelectPaciente(paciente.id)}
                >
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-14 w-14 bg-gradient-to-br from-emerald-500 to-teal-600 border-2 border-white shadow-sm">
